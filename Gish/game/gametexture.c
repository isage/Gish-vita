...
void loadtilesettemp(void)
  ...
  for (count=0;count<256;count++)
    ...
    if (game.levelnum!=6)
      loadtexturetga_internal(count+1792,texfilename,0,GL_CLAMP_TO_EDGE,GL_CLAMP_TO_EDGE,GL_LINEAR,GL_LINEAR, filepath);
    else
      loadtexturetga_internal(count+1792,texfilename,0,GL_CLAMP_TO_EDGE,GL_CLAMP_TO_EDGE,GL_NEAREST,GL_NEAREST, filepath);
    }
