...
void edittextures(void)
{
  ...
  while (!menuitem[0].active && !windowinfo.shutdown)
  {
    ...
    for (count = 0; count < 32; count++)
    {
      ...
      GLfloat quadA[] = {0, 0, -1.0f, 0, 0, -1.0f, 0, 0, -1.0f, 0, 0, -1.0f};
      ...
      if (texture[count + textureedit.pagenum * 32].filename[0])
      {
        ...
        drawtext(cleanFilename, ((count & 7) * 64 + 32) | TEXT_CENTER, 14 + 60 + (count >> 3) * 64, 8, 1.0f, 1.0f, 1.0f,
                 1.0f);
      }
    }
    for (count = 0; count < 16; count++)
    {
      ...
      glEnableClientState(GL_VERTEX_ARRAY);
      glEnableClientState(GL_TEXTURE_COORD_ARRAY);
      GLfloat quadB[] = {0, 0, -1.0f, 0, 0, -1.0f, 0, 0, -1.0f, 0, 0, -1.0f};
      GLfloat txB[]   = {0, 0, 1, 0, 1, 1, 0, 1};
#else
      glBegin(GL_QUADS);
#endif
      vec[0] = (count & 7) * 64 + 4;
    ...
    if (textureedit.texturenum != -1)
    {
      ...
      glEnableClientState(GL_VERTEX_ARRAY);
      glEnableClientState(GL_TEXTURE_COORD_ARRAY);
      GLfloat quadC[] = {0, 0, -1.0f, 0, 0, -1.0f, 0, 0, -1.0f, 0, 0, -1.0f};
      GLfloat txC[]   = {0, 0, 1, 0, 1, 1, 0, 1};
#else
      glBegin(GL_QUADS);
#endif
      vec[0] = mouse.x - 28;
    ...
    for (count = 0; count < (texture[texturenum].sizey >> mipmaplevel); count++)
      for (count2 = 0; count2 < (texture[texturenum].sizex >> mipmaplevel); count2++)
        texture[texturenum].rgba[mipmaplevel][count * (texture[texturenum].sizex >> mipmaplevel) + count2]
            = texture[oldtexturenum].rgba[mipmaplevel][count * (texture[oldtexturenum].sizex >> mipmaplevel) + count2];
  }
...
void loadtilesettemp(void)
{
  ...
  if (textureedit.tileset == 0)
    strcpy(filepath, "tile01/texture/");
  if (textureedit.tileset == 1)
    strcpy(filepath, "tile02/texture/");
  if (textureedit.tileset == 2)
    strcpy(filepath, "tile03/texture/");
  if (textureedit.tileset == 3)
    strcpy(filepath, "tile04/texture/");
  if (textureedit.tileset == 4)
    strcpy(filepath, "tile05/texture/");
  if (textureedit.tileset == 5)
    strcpy(filepath, "tile06/texture/");
  ...
  if (textureedit.tileset == 7)
    ...
    for (count = 0; count < 256; count++)
    {
      ...
      if (game.levelnum != 6)
        loadtexturetga_internal(count + 1792, texfilename, 0, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE, GL_LINEAR, GL_LINEAR,
                                filepath);
      else
        loadtexturetga_internal(count + 1792, texfilename, 0, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE, GL_NEAREST,
                                GL_NEAREST, filepath);
    }
}
