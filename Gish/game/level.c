  ...
  */
  ...
  if ((fp = fopen(filepath, "wb")) != NULL)
  {
    ...
    for (count = 1; count < 251; count++)
    {
      if (textureused[count])
      {
        if (debug_level_saveload)
          printf("Saving %i as ", count);
...
void loadlevel(char *filename)
{
  ...
  if ((fp = fopen(filepath, "rb")) != NULL)
  {
    ...
    if (version == 9)
    {
      ...
      if (!lighting_enabled)
      {
        if ((strcmp(filename, "cave6.lvl") == 0) || (strcmp(filename, "twilight5.lvl") == 0))
        {
          float ambient[4][3] = {{0.150000, 0.250000, 0.150000},
...
void loadleveltextures(void)
{
  ...
  for (count = 0; count < 251; count++)
  {
    ...
    if (game.levelnum != 6)
      loadtexturetga_internal(count, texfilename, 0, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE, GL_LINEAR, GL_LINEAR,
                              filepath);
    else
      loadtexturetga_internal(count, texfilename, 0, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE, GL_NEAREST, GL_NEAREST,
                              filepath);
  }
}

int lineintersectline3(float *intersectpoint, float *normal, float *scale, float *startpoint, float *endpoint,
