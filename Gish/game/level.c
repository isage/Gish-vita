...
void loadleveltextures(void)
  ...
  for (count=0;count<251;count++)
    ...
    if (game.levelnum!=6)
      loadtexturetga_internal(count,texfilename,0,GL_CLAMP_TO_EDGE,GL_CLAMP_TO_EDGE,GL_LINEAR,GL_LINEAR, filepath);
    else
      loadtexturetga_internal(count,texfilename,0,GL_CLAMP_TO_EDGE,GL_CLAMP_TO_EDGE,GL_NEAREST,GL_NEAREST, filepath);
    }
