...
void objectcycle(void)
{
  ...
  for (count = 0; count < numofropes; count++)
  {
    ...
    {
      bond[rope[count].bondnum].length
          = bond[rope[count].bondnum].maxlength + (rope[count].range - cos(rope[count].angle) * rope[count].range);
  ...
  for (count = 0; count < numofobjects; count++)
  {
    ...
    if (object[count].type == 9)
      ...
      {
        vec[1] = particle[object[count].particle[6]].position[1] - 0.375f
                 - particle[object[count].particle[4]].position[1];
        vec[1] *= 0.08f;
        vec[1] -= particle[object[count].particle[4]].velocity[1];
        particle[object[count].particle[4]].velocity[1] += vec[1];

        vec[1] = particle[object[count].particle[6]].position[1] - 0.375f
                 - particle[object[count].particle[5]].position[1];
        vec[1] *= 0.08f;
        vec[1] -= particle[object[count].particle[5]].velocity[1];
        particle[object[count].particle[5]].velocity[1] += vec[1];
      }
    ...
    if (object[count].type == 15)
    {
      if (object[count].link == -1 || object[object[count].link].idata[0] == 1)
        ...
        {
          createwheel(object[count].position, object[count].size[0], object[count].size[1], object[count].mass, 0.8f,
                      0);
          ...
          {
            particle[object[numofobjects - 1].particle[count2]].velocity[0]
                = object[count].lightcolor[0] / PHYSICSCYCLE;
            particle[object[numofobjects - 1].particle[count2]].velocity[1]
                = object[count].lightcolor[1] / PHYSICSCYCLE;
          }
        }
    }
    ...
    if (object[count].type == 10)
    {
      ...
      {
        vec[count3] = particle[object[count].particle[4]].position[count3]
                      - particle[object[count].particle[count2]].position[count3];
        ...
        if (particle[object[count].particle[count2]].position[count3]
            > particle[object[count].particle[4]].position[count3] + 0.25f)
          particle[object[count].particle[count2]].position[count3]
              = particle[object[count].particle[4]].position[count3] + 0.25f;
        if (particle[object[count].particle[count2]].position[count3]
            < particle[object[count].particle[4]].position[count3] - 0.25f)
      ...
      vec[0]
          = particle[object[count].particle[4]].position[count3] - particle[object[count].particle[1]].position[count3];
    ...
    if (object[count].type == 1)
    {
        ...
        if (x > 0 && x < 254)
          if (y > 0 && y < 254)
          {
            if (block[level.foregrid[y][x]].density != 0.0f)
              if ((rand() & 255) == 0)
              {
                createparticle(5, object[count].position, object[count].velocity, 0.125f * 0.25f, -1,
                               100 + (rand() & 63));
            ...
            if ((level.grid[y][x] >= 248 /*&& level.grid[y][x]<=255*/ && level.gridmod[y][x] == 0)
                || (level.grid[y][x] >= 240 && level.grid[y][x] <= 247 && level.gridmod[y][x] == 2))
            {
              vec[0] = (float)x + 0.5f;
              vec[1] = (float)y + 0.5f;
              vec[2] = 0.0f;
      ...
      for (count2 = 0; count2 < 16; count2++)
        for (count3 = 0; count3 < 16; count3++)
          ...
          {
            subtractvectors(vec, particle[object[count].particle[count3]].position,
      ...
      spin  = cos(angle) * fabs(cos(angle)) * object[count].axis[0]
             + sin(angle) * fabs(sin(angle)) * object[count].axis[1];
      ...
      */
      ...
      for (count2 = 0; count2 < 16; count2++)
      {
        ...
        if (object[count].numoforientations != 0)
        {
          scaleaddvectors(particle[object[count].particle[count2]].velocity,
                          particle[object[count].particle[count2]].velocity, vec2, spin * 0.004f);
        }

        // if (object[count].numoforientations==0)
        {
          scaleaddvectors(particle[object[count].particle[count2]].velocity,
                          particle[object[count].particle[count2]].velocity, xaxis, object[count].axis[0] * 0.001f);
          if (object[count].axis[1] < 0.0f)
            scaleaddvectors(particle[object[count].particle[count2]].velocity,
                            particle[object[count].particle[count2]].velocity, yaxis, object[count].axis[1] * 0.001f);
          else
            scaleaddvectors(particle[object[count].particle[count2]].velocity,
                            particle[object[count].particle[count2]].velocity, yaxis, object[count].axis[1] * 0.0005f);
        }
      }
    ...
    {
      subtractvectors(object[count].orientation[1], particle[object[count].particle[0]].position,
      ...
      for (count2 = 0; count2 < 4; count2++)
      {
        ...
        if (count2 == 1 || count2 == 2)
        {
          ...
          subtractvectors(particle[object[count].particle[count2]].velocity,
                          particle[object[count].particle[count2]].velocity, vec2);
        }
        else
        {
    ...
    if (object[count].type == 4)
    {
      ...
      if (object[count].beasttype == 0 || object[count].beasttype == 2 || object[count].beasttype == 8
          ...
          if ((object[count].button & 2) == 2)
            for (count2 = 0; count2 < 4; count2++)
            {
              if (object[count].beasttype != 8)
                scaleaddvectors(particle[object[count].particle[count2]].velocity,
                                particle[object[count].particle[count2]].velocity, yaxis, 0.08f);
              else
                scaleaddvectors(particle[object[count].particle[count2]].velocity,
                                particle[object[count].particle[count2]].velocity, yaxis, 0.12f);
            }
      if (object[count].beasttype == 15)
        if (object[count].numoforientations != 0)
          if ((object[count].button & 2) == 2)
      ...
      if (object[count].numoforientations != 0)
        if (dotproduct(object[count].orientation[1], yaxis) < 0.7f)
        {
          ...
          for (count2 = 0; count2 < 4; count2++)
          {
            ...
            scaleaddvectors(particle[object[count].particle[count2]].velocity,
                            particle[object[count].particle[count2]].velocity, vec2, spin * 0.003f);
          }
        }
    }
  }
