...
void editlevelobjects(void)
{
  ...
  while (!menuitem[0].active && !windowinfo.shutdown)
  {
    ...
    if (!menuinputkeyboard)
    {
      ...
      {
        vec[0] = 1.0f;
        if (level.object[editor.objectnum].type == 6 || level.object[editor.objectnum].type == 7
            || level.object[editor.objectnum].type == 17)
          vec[0] = 0.2f;
...
void renderlevelobjects(void)
{
  ...
  for (count = 0; count < level.numofobjects; count++)
  {
    ...
    if (level.object[count].type >= 2 && level.object[count].type <= 5)
    {
      ...
      glVertex3f(level.object[count].position[0] - level.object[count].size[0] * 0.5f,
                 level.object[count].position[1] + level.object[count].size[1] * 0.5f, 0.0f);
