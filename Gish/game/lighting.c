...
void rendershadows(void)
{
  ...
  for (lightcount = 0; lightcount < frame.numoflights; lightcount++)
  {
    ...
    for (count = view.position[1] - (view.zoomy + lightrange); count <= view.position[1] + (view.zoomy + lightrange);
         count++)
      if (count >= 0 && count < 256)
        for (count2 = view.position[0] - (view.zoomx + lightrange);
             count2 <= view.position[0] + (view.zoomx + lightrange); count2++)
      ...
      if (object[count].timetolive >= 50)
      {
        if (object[count].type == 2)
        {
          for (count2 = 0; count2 < 4; count2++)
          {
            ...
            if (dotproduct(vec2, normal) < 0.0f)
            {
              subtractvectors(vec3, frame.light[lightcount].position,
              ...
              glVertex3fv(vec);
#endif
              scaleaddvectors(vec, particle[object[count].particle[((count2 + 1) & 3)]].position, vec3,
                              -frame.light[lightcount].intensity * 0.55f);
#if defined(GLES)
              quad[6] = vec[0];
              quad[7] = vec[1];
              quad[8] = vec[2];
              ...
              glVertex3fv(vec);
#endif
              scaleaddvectors(vec, particle[object[count].particle[count2]].position, vec2,
                              -frame.light[lightcount].intensity * 0.55f);
#if defined(GLES)
              quad[9]  = vec[0];
              quad[10] = vec[1];
...
void renderobjectspecular(int objectnum)
{
  ...
  for (lightcount = 0; lightcount < frame.numoflights; lightcount++)
  {
    ...
    for (count = 0; count < 32; count++)
    {
      ...
      vec2[0] = objectrender[objectnum].vertex[((count + 0) & 31)][1]
                - objectrender[objectnum].vertex[((count + 1) & 31)][1];
      vec2[1] = objectrender[objectnum].vertex[((count + 1) & 31)][0]
                - objectrender[objectnum].vertex[((count + 0) & 31)][0];
      vec2[2] = 0.0f;
      vec[0] = objectrender[objectnum].vertex[((count - 1) & 31)][1]
               - objectrender[objectnum].vertex[((count + 0) & 31)][1];
      vec[1] = objectrender[objectnum].vertex[((count + 0) & 31)][0]
               - objectrender[objectnum].vertex[((count - 1) & 31)][0];
      vec[2] = 0.0f;
      normalizevector(vec2, vec2);
      normalizevector(vec, vec);
      ...
      vec2[0] = objectrender[objectnum].vertex[((count + 1) & 31)][1]
                - objectrender[objectnum].vertex[((count + 2) & 31)][1];
      vec2[1] = objectrender[objectnum].vertex[((count + 2) & 31)][0]
                - objectrender[objectnum].vertex[((count + 1) & 31)][0];
      vec2[2] = 0.0f;
      vec[0] = objectrender[objectnum].vertex[((count + 0) & 31)][1]
               - objectrender[objectnum].vertex[((count + 1) & 31)][1];
      vec[1] = objectrender[objectnum].vertex[((count + 1) & 31)][0]
               - objectrender[objectnum].vertex[((count + 0) & 31)][0];
      vec[2] = 0.0f;
      normalizevector(vec2, vec2);
      normalizevector(vec, vec);
