...
int loadtexturetga_app(int texturenum, char *filename, int mipmap, int wraps, int wrapt, int magfilter, int minfilter)
{
  ...
  strcpy(filepath, "app0:/data/");
  strcat(filepath, filename);

  if (debug_texture_load)
    printf("Loading \"%s\" into #%i...\n", filename, texturenum);

  if ((fp = fopen(filepath, "rb")) == NULL)
  {
    if (debug_texture_load)
      printf("Texture #%d \"%s\" failed: fopen error\n", texturenum, filename);
  ...
  {
    if (debug_texture_load)
      printf("Texture #%d \"%s\" failed: bad format\n", texturenum, filename);
  ...
  for (count = 0; count < tgaheader.imageheight; count++)
    for (count2 = 0; count2 < tgaheader.imagewidth; count2++)
    {
      ...
      {
        if (origin == 0)
          tgaheader.imagedata[(tgaheader.imageheight - 1 - count) * tgaheader.imagewidth + count2]
              = (alpha << 24) + (blue << 16) + (green << 8) + red;
        if (origin == 1)
          tgaheader.imagedata[(tgaheader.imageheight - 1 - count) * tgaheader.imagewidth
                              + (tgaheader.imagewidth - 1 - count2)]
              = (alpha << 24) + (blue << 16) + (green << 8) + red;
        if (origin == 2)
          tgaheader.imagedata[count * tgaheader.imagewidth + count2]
              = (alpha << 24) + (blue << 16) + (green << 8) + red;
        if (origin == 3)
          tgaheader.imagedata[count * tgaheader.imagewidth + (tgaheader.imagewidth - 1 - count2)]
              = (alpha << 24) + (blue << 16) + (green << 8) + red;
      }
      else
      {
...
{
  return loadtexturetga_internal(texturenum, filename, mipmap, wraps, wrapt, magfilter, minfilter, "texture/");
}

int loadtexturetga_anim(int texturenum, char *filename, int mipmap, int wraps, int wrapt, int magfilter, int minfilter)
{
  return loadtexturetga_internal(texturenum, filename, mipmap, wraps, wrapt, magfilter, minfilter, "animation/");
}
  ...
  for (count = 0; count < texture[texturenum].sizey; count++)
    if (count < tgaheader.imageheight)
      for (count2 = 0; count2 < texture[texturenum].sizex; count2++)
        ...
        {
          texture[texturenum].rgba[0][count * texture[texturenum].sizex + count2]
              = tgaheader.imagedata[(starty + count) * tgaheader.imagewidth + (startx + count2)];
          if (!bigendian)
          {
...
void generatemipmap(int texturenum)
{
  ...
  for (mipmaplevel = 1; mipmaplevel < mipmaplevelmax; mipmaplevel++)
  {
    ...
    texture[texturenum].rgba[mipmaplevel] = (unsigned int *)malloc((texture[texturenum].sizex >> mipmaplevel)
                                                                   * (texture[texturenum].sizey >> mipmaplevel) * 4);
    if (texture[texturenum].normalmap)
    {
      free(texture[texturenum].normal[mipmaplevel]);
      texture[texturenum].normal[mipmaplevel] = (unsigned int *)malloc(
          (texture[texturenum].sizex >> mipmaplevel) * (texture[texturenum].sizey >> mipmaplevel) * 4);
    }
    for (count = 0; count < (texture[texturenum].sizey >> mipmaplevel); count++)
      for (count2 = 0; count2 < (texture[texturenum].sizex >> mipmaplevel); count2++)
      {
        ...
        for (count3 = 0; count3 < 2; count3++)
          ...
          {
            temp = texture[texturenum]
                       .rgba[mipmaplevel - 1][(count * 2 + count3) * (texture[texturenum].sizex >> (mipmaplevel - 1))
                                              + (count2 * 2 + count4)];
            red += temp & 0xFF;
            green += (temp >> 8) & 0xFF;
            blue += (temp >> 16) & 0xFF;
        ...
        blue >>= 2;
        alpha >>= 2;
        texture[texturenum].rgba[mipmaplevel][count * (texture[texturenum].sizex >> mipmaplevel) + count2]
            = (alpha << 24) + (blue << 16) + (green << 8) + red;
        ...
        if (texture[texturenum].normalmap)
        {
          ...
          for (count3 = 0; count3 < 2; count3++)
            for (count4 = 0; count4 < 2; count4++)
            {
              temp
                  = texture[texturenum]
                        .normal[mipmaplevel - 1][(count * 2 + count3) * (texture[texturenum].sizex >> (mipmaplevel - 1))
                                                 + (count2 * 2 + count4)];
              red += temp & 0xFF;
              green += (temp >> 8) & 0xFF;
              blue += (temp >> 16) & 0xFF;
          ...
          blue >>= 2;
          alpha >>= 2;
          texture[texturenum].normal[mipmaplevel][count * (texture[texturenum].sizex >> mipmaplevel) + count2]
              = (alpha << 24) + (blue << 16) + (green << 8) + red;
        }
      }
  }
...
void setuptexture(int texturenum)
{
  ...
  {
    glTexImage2D(GL_TEXTURE_2D, count, texture[texturenum].format, (texture[texturenum].sizex >> count),
                 (texture[texturenum].sizey >> count), 0, texture[texturenum].format, GL_UNSIGNED_BYTE,
                 texture[texturenum].rgba[count]);
    // printf("GL RES %x\n", glGetError());
  }
  ...
  if (texture[texturenum].normalmap)
  {
    ...
    for (count = 0; count < texture[texturenum].mipmaplevels; count++)
    {
      glTexImage2D(GL_TEXTURE_2D, count, GL_RGBA, (texture[texturenum].sizex >> count),
                   (texture[texturenum].sizey >> count), 0, GL_RGBA, GL_UNSIGNED_BYTE,
                   texture[texturenum].normal[count]);
      // printf("GL RES %x\n", glGetError());
    }
  }
  if (texture[texturenum].glossmap)
  {
    ...
    for (count = 0; count < texture[texturenum].mipmaplevels; count++)
    {
      glTexImage2D(GL_TEXTURE_2D, count, GL_ALPHA, (texture[texturenum].sizex >> count),
                   (texture[texturenum].sizey >> count), 0, GL_ALPHA, GL_UNSIGNED_BYTE,
                   texture[texturenum].gloss[count]);
      // printf("GL RES %x\n", glGetError());
    }
  }
}
